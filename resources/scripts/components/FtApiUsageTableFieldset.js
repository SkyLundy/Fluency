...
const initializeApiUsageTableFieldset = function (fieldset) {
  ...
  this.updateView = () => {
    ...
    Fluency.getUsage().then(result => {
      ...
      usageTableElements.limit.innerText = formatNumber(result.limit);
      usageTableElements.remaining.innerText = formatNumber(result.remaining);
      usageTableElements.used.innerText = formatNumber(result.used);
