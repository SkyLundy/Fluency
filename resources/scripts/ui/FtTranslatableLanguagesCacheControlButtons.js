...
const FtTranslatableLanguageCacheControlButtons = (function () {
  ...
  const init = () => {
    const clearCacheButtons = [
      ...document.querySelectorAll('.js-ft-clear-translatable-languages-cache'),
    ];
...
const clearCacheButton = function (button) {
  ...
  this.bindEventListener = () => {
    ...
    button.addEventListener('click', e => {
      ...
      Fluency.deleteTranslatableLanguagesCache().then(result => {
        if (result.error) {
...
export default FtTranslatableLanguageCacheControlButtons;
