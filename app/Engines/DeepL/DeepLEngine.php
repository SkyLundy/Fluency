...
use Fluency\{FluencyErrors, FluencyLocalization};
...
final class DeepLEngine implements FluencyEngine
{
    ...
    public function getLanguages(): EngineTranslatableLanguagesData
    {
        ...
        $getSource = function (string $targetCode) use ($sourceResult, &$getSource): ?object {
            ...
            if (
                $sourceLanguage ||
                !$sourceLanguage && strlen($targetCode) === 2
            ) {
    ...
    private function addIgnoredTags(array $texts, array|string $ignoredStrings = []): array
    {
        $ignoredStrings = [
            ...
            $this->globalIgnoredStrings,
        ];

        $ignoredStrings = array_reduce($ignoredStrings, function($ignoredArr, $ignoredString) {
            $ignoredString = preg_replace('/\n/', '||', $ignoredString);

            return $ignoredArr = [
                ...$ignoredArr,
                ...explode('||', $ignoredString),
            ];
        }, []);
        ...
        // Sort by longest strings first to prevent substring matches before full string matches
        usort($ignoredStrings, fn ($a, $b) => strlen($b) <=> strlen($a));

        $insertTags = function(string $text) use ($ignoredStrings) {
            return array_reduce($ignoredStrings, function ($text, $ignored) {
                $pattern = preg_quote($ignored);

                $text = preg_replace("/{$pattern}/U", "<span translate=\"no\">{$ignored}</span>", $text);
                return $text;
            }, $text);
        };

        return array_map($insertTags, $texts);
    }
